                ...
                <v-table>
                    ...
                    <tbody>
                        <tr v-for="(val, idx) in value" :key="val" @click="changeSelectedRow(val)" :style="val === selectedRow ? 'background-color: #f0f3ff;':''">
                            ...
                            <td class="whitespace-nowrap" label="멤버">{{ val.members }}</td>
                            <td class="whitespace-nowrap" label="메시지 목록">{{ val.messages }}</td>
        ...
        <v-col>
            ...
            >
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            <UUID offline label="채팅방 ID" v-model="selectedRow.chatroomId" :editMode="true"/4>
